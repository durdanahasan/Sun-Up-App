                    ...
                    ForEach(viewModel.orders) { order in
                        NavigationLink { OrderDetailView(order: order) } label: {
                            VStack(alignment: .leading, spacing: 12) {
                                HStack { Image(systemName: "beach.umbrella.fill").frame(width: 54, height: 54).background(SunUpTheme.yellow.opacity(0.25)).clipShape(RoundedRectangle(cornerRadius: 12)); Text(order.beach.name).font(.sunUpCardTitle()); Spacer(); Text(order.totalAED.aed).font(.headline).foregroundStyle(SunUpTheme.teal) }
